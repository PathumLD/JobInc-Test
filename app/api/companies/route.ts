...
export async function GET(request: NextRequest): Promise<NextResponse<CompaniesResponse>> {
  try {
    ...
    // Fetch companies that this MIS user can create jobs for
    const { getCompaniesForMisUser } = await import('@/lib/db/companies');
    const companies = await getCompaniesForMisUser(payload.userId);
